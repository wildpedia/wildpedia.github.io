...
const I18n = (() => {
  ...
  async function init() {
    _lang = localStorage.getItem('ak-lang') || 'en';
  ...
  function getAnimalName(animal) {
    if (animal.name[_lang]) return animal.name[_lang];
    return animal.name.en || animal.id;
  }
  ...
  async function setLang(lang) {
    ...
    _applyTranslations();
    _updateSwitcher();
    document.dispatchEvent(new CustomEvent('ak-lang-change', { detail: { lang: _lang } }));
  }
  ...
  return { init, t, getLang, getManifest, getAnimalName, setLang };
})();
